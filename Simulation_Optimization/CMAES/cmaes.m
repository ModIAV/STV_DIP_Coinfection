function [xmin,fmin, counteval,stopflag,out,bestever] = cmaes(fitfun, xstart, insigma, inopts,varargin ) 
...
%                         num2str(sigma*max(diagD))]);
        ...
        if fitness.hist(1) < out.solutions.bestever.f
            ...
            bestever = out.solutions.bestever;
            save bestever.mat bestever   % Joao Ramos
        end
        
        % Set stop flag
%%% Change
if stopOptimization, stopflag(end+1) = {'maxtime'}; end
...
%                 disp(['Iterat, #Fevals,    Time:   Function Value    (median,worst) ' ...
%                     '|Axis Ratio|' ...
...
disp([repmat(' ',1,4-floor(log10(countiter))) ...
    ...
    repmat(' ',1,5-floor(log10(counteval))) ...
    num2str(counteval) ' , ' ...
    repmat(' ',1,5-floor(log10(toc(tstart0)))) ...
    num2str( toc(tstart0) , '%.1f') ' : ' ...
    repmat(' ',1,3) ...
    num2str(bestever.f, '%.9e') ... % fitness.hist(1)
    ' +(' num2str(median(fitness.raw)-fitness.hist(1), '%.0e ') ...
    ',' num2str(max(fitness.raw)-fitness.hist(1), '%.0e ') ...
    ')  | ' ...
    num2str(max(diagD)/min(diagD), '%4.2e') ' | ' ...
    ...
    num2str(maxstd, ' %.1e')]);
                ...
                xbest=xmean; save xbest.mat xbest  % Joao Ramos
            end
        end
